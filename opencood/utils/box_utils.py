...
def project_world_objects(object_dict,
    ...
    for object_id, object_content in object_dict.items():
        ...
        if bbx_lidar.shape[0] > 0 and cav_lidar_pose is not None:
            center_xyz = bbx_lidar[0, :3]
            cav_lp = np.array(cav_lidar_pose)
            if cav_lp.ndim == 1:
                dist = np.linalg.norm(center_xyz - cav_lp[:3])
            else:
                dist = np.min(
                    np.linalg.norm(cav_lp[:, :3] - center_xyz.reshape(1, 3),
                                   axis=1))
            if dist >= 3:
                continue

        if bbx_lidar.shape[0] > 0:
            if cav_label_list is not None and str(object_id) not in cav_label_list:
                continue
...
def _bev_from_3d_boxes(boxes3d):
    assert boxes3d.ndim == 3 and boxes3d.shape[1:] == (8, 3), \
        f"Expected boxes with shape (N, 8, 3), got {tuple(boxes3d.shape)}"
    return boxes3d[:, :4, :2].contiguous()


def _to_obb_simple(bev_boxes):
    centers = bev_boxes.mean(dim=1)
    edge1 = bev_boxes[:, 1] - bev_boxes[:, 0]
    edge2 = bev_boxes[:, 2] - bev_boxes[:, 1]
    length1 = edge1.norm(dim=1)
    length2 = edge2.norm(dim=1)

    use_edge1 = length1 >= length2
    direction = torch.where(use_edge1.unsqueeze(1), edge1, edge2)
    width = torch.where(use_edge1, length1, length2)
    height = torch.where(use_edge1, length2, length1)
    angle = -torch.atan2(direction[:, 1], direction[:, 0])
    angle = angle.remainder(np.pi)
    angle = torch.where(angle >= np.pi / 2, angle - np.pi, angle)

    return torch.stack(
        [centers[:, 0], centers[:, 1], width, height, angle], dim=1)


def nms_rotated_gpu(boxes3d, scores, iou_threshold, top=3000):
    """Run rotated BEV NMS with MMCV CUDA ops."""
    if boxes3d.numel() == 0:
        return np.array([], dtype=np.int32)
    if not boxes3d.is_cuda or not scores.is_cuda:
        raise ValueError("GPU NMS requires CUDA boxes and scores")

    _, sorted_indices = torch.sort(scores, descending=True)
    sorted_indices = sorted_indices[:top]
    boxes3d_top = boxes3d[sorted_indices]
    scores_top = scores[sorted_indices]
    oriented_boxes = _to_obb_simple(_bev_from_3d_boxes(boxes3d_top))

    _, keep_indices = mmcv_nms_rotated(
        oriented_boxes, scores_top, iou_threshold, clockwise=False)
    return sorted_indices[keep_indices.long()].cpu().numpy().astype(np.int32)
