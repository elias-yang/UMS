...
class IntermediateFusionDataset(basedataset.BaseDataset):
    def __init__(self, params, visualize, train=True, isSim=False,
        ...
        super(IntermediateFusionDataset, self). \
            __init__(params, visualize, train, isSim, ego_only, nofusion,
    ...
    def __getitem__(self, idx):
        ...
        for i, (cav_id, selected_cav_base) in enumerate(base_data_dict.items()):
            if self.ego_only and not selected_cav_base['ego']:
                continue
            ...
            if self.cav_label_only:
                if self.isSim:
                    cav_label_list = base_data_dict.keys()
                    cav_lidar_pose = None
                else:
                    cav_label_list = None
                    cav_lidar_pose = [
                        cav_content['params']['transformation_matrix'][:3, 3]
            ...
            selected_cav_processed, void_lidar = self.get_item_single_car(
                selected_cav_base,
                ego_lidar_pose,
                cav_label_list=cav_label_list,
                cav_lidar_pose=cav_lidar_pose,
                ego_id=ego_id)
            ...
            object_type_stack += selected_cav_processed['object_type']
            if self.nofusion and not selected_cav_base['ego']:
                continue
        ...
        processed_data_dict['ego'].update(
            {'object_bbx_center': object_bbx_center,
             ...
             'object_type': [object_type_stack[i] for i in unique_indices],
             ...
             'unique_id': f'{base_data_dict[ego_id]["scenario_index"]}_{ego_id}_{base_data_dict[ego_id]["timestamp_key"]}'})
        ...
        if self.visualize:
            ...
            if self.single_pc:
                processed_data_dict['ego'].update(
                    {'single_pc_lidar': projected_lidar_stack})
        return processed_data_dict

    def get_item_single_car(self, selected_cav_base, ego_pose,
                            cav_label_list=None, cav_lidar_pose=None,
                            ego_id=0):
        """
        ...
        object_bbx_center, object_bbx_mask, object_ids, object_type = \
            self.post_processor.generate_object_center([selected_cav_base],
                                                       transformation_matrix if not self.isSim else ego_pose,
                                                       cav_label_list=cav_label_list,
                                                       cav_lidar_pose=cav_lidar_pose)

        if self.pseudo_labels is not None and self.use_pseudo and self.train:
            unique_id = f'{selected_cav_base["scenario_index"]}_{ego_id}_{selected_cav_base["timestamp_key"]}'
            if unique_id in self.pseudo_labels and len(self.pseudo_labels[unique_id]['gt_boxes']) > 0:
                pseudo_bbx = self.pseudo_labels[unique_id]['gt_boxes']
                pseudo_centers = box_utils.corner_to_center(
                    pseudo_bbx, order=self.params['postprocess']['order'])
                if self.ego_only:
                    keep_mask = np.ones(pseudo_centers.shape[0], dtype=bool)
                    for i, box in enumerate(pseudo_centers):
                        x, y = box[0], box[1]
                        if abs(x) < 1.5 and abs(y) < 1.5:
                            keep_mask[i] = False
                    pseudo_centers = pseudo_centers[keep_mask]

                object_bbx_center = np.zeros(
                    (self.params['postprocess']['max_num'], 7))
                object_bbx_mask = np.zeros(
                    self.params['postprocess']['max_num'])
                object_bbx_center[:pseudo_centers.shape[0], :] = pseudo_centers
                object_bbx_mask[:pseudo_centers.shape[0]] = 1
                object_ids = [i for i in range(pseudo_centers.shape[0])]
                object_type = ['Car'] * pseudo_centers.shape[0]
            else:
                max_num = self.params['postprocess']['max_num']
                object_bbx_center = np.zeros((max_num, 7))
                object_bbx_mask = np.zeros(max_num)
                object_ids = []
                object_type = []
        ...
        object_type = list(np.array(object_type)[range_mask])
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            object_type.append(ego_dict['object_type'])
            ...
            if 'unique_id' in ego_dict:
                unique_ids.append(ego_dict['unique_id'])
            if self.visualize:
        ...
        if self.visualize:
            ...
            if self.single_pc:
                single_pc_lidar = []
                for i in range(len(batch)):
                    single_pc_lidar.append(batch[i]['ego']['single_pc_lidar'])
                output_dict['ego'].update({'single_pc_lidar': single_pc_lidar})
        return output_dict
    ...
    def load_pseudo_labels(self, pseudo_labels):
        self.pseudo_labels = pseudo_labels

    def set_use_pseudo(self, use_pseudo):
